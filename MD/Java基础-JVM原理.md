...
## 垃圾收集器
![image](https://github.com/studentytj/Java-Interview-update/assets/16250682/ab8bf8e4-819b-4bd0-b02c-1b7f19e7513a)
以上是 HotSpot 虚拟机中的 7 个垃圾收集器，连线表示垃圾收集器可以配合使用。单线程与多线程: 单线程指的是垃圾收集器只使用一个线程进行收集，而多线程使用多个线程；串行与并行: 串行指的是垃圾收集器与用户程序交替执行，这意味着在执行垃圾收集的时候需要停顿用户程序；并形指的是垃圾收集器和用户程序同时执行。除了 CMS 和 G1 之外，其它垃圾收集器都是以串行的方式执行。
> 注意：要查看java8对应使用的是哪种收集器，可以在启动参数添加`-XX:+PrintCommandLineFlags`，发现启动时打印的参数为`-XX:+UseParallelGC`：即使用parallel scavenge & parallel old搜集器
